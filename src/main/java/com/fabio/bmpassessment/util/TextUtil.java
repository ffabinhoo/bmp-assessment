...
public class TextUtil {
	...
	public static String[] getWordsFromText(){
		String[] words = TextUtil.PARAGRAPH.replaceAll("\\p{Punct}", "").toLowerCase().trim().split("\\s+");
		return words;
 	}
	
	
	public static Map<String, Integer> searchByWord(String[] searchText) {
		Map<String, Integer> wordCounts = new HashMap<String, Integer>();
		
		//initializing wordCounts with inputs
		for (int i = 0; i < searchText.length; i++) {
			wordCounts.put(searchText[i], 0);
		}
		...
		for (int i = 0; i < wordsFromText.length; i++) {
			for (String searchWord : searchText) {
				if (wordsFromText[i].toLowerCase().equals(searchWord.toLowerCase())) {
					wordCounts.put(searchWord, wordCounts.getOrDefault(searchWord, 0) + 1);
				}
			}
		}
		return 	wordCounts;
	...
	public static Map<String, Integer> generateWordCounts() {
		Map<String, Integer> wordCounts = new HashMap<String, Integer>();
		String[] wordsFromTest = TextUtil.getWordsFromText();
		
		for (String word : wordsFromTest) {
